...
$id = $cr->id;
    ...
    $QUERY_insert_CR = 'INSERT into COMPTERENDU (id_cr,content, datepub, auteur) values('.$id.',\''.$contenue.'\',\''.$date.'\',\'export\');';
...
function download(string $url, string $file_name,int $id,string $extension){
    ...
    if(file_exists($file_name)){
        ...
        if(file_put_contents( $file_name,file_get_contents($url))) {
            ...
            $QUERY_insert_DOCUMENT = 'INSERT into DOCUMENT (typedoc, nom, chemin) values(\''.$extension.' \',\''.$file_name.'\',\'Compte-rendu/'.$file_name.'.'.$extension.'\');';
